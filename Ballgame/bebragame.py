...
class myobject():  # навзание неинформативно
    ...
    def __init__(self, surface, k, type): # сделать подклассы у которых в ините задавать k и type
        """конструктор"""
...
def click_object(event, obj):
    ...
    if((obj.x-event.pos[0])**2 + (obj.y-event.pos[1])**2 <= obj.r**2):
        #  сделать типы не цирфами а строчками
        if obj.type == 2:
            finished = True
        elif obj.type == 0:
            table1.points += 1
...
class Ball():  
    ...
    def __init__(self): # сделать подклассы у которых в ините задавать k и type
        """конструктор"""
        global Xbound, Ybound
...
# ?????????
# функцию рисования запихнуть в классе балл1 квадрат1 и квадрат2
#вообще глобальные переменные запрещены под страхом анальной казни(лучше все писать внутри классов)
...
# наброски касательно коллизии между шарами
def smashballs():
    global all_objects
